...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Multicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Multicoin Signed Message:\n";
...
{
    // Multicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60; // Multicoin: 1 minute
static const int64 nTargetSpacing = 60; // Multicoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Multicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Multicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Multicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xc0, 0xfb }; // Multicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MulticoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MulticoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MulticoinMiner(CWallet *pwallet)
{
    printf("MulticoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("multicoin-miner");
    ...
    try { loop {
        ...
        printf("Running MulticoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MulticoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MulticoinMiner, pwallet));
}
